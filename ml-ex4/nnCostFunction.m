...
tempy = zeros(m, num_labels);   %5000 x 10
for i = 1: m
  tempy(i, y(i)) = 1;
end
J = sum(sum(-tempy.*log(a_3) - (1.- tempy).*log(1.- a_3))) / m;
...
delta3 = a_3 - tempy;%5000x10

delta2 = (delta3* Theta2 ).* (a_2.*(1.-a_2)); %Theta2: 10x26
delta2 = delta2(:, 2 : end);
Theta1_grad = Theta1_grad + delta2'* X;
Theta1_grad = Theta1_grad / m + lambda * Theta1 / m;%25x401
